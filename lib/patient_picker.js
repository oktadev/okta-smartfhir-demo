...
module.exports.getHandler = async (cookieString) => {
	...
	try {
		...
		const client = new okta.Client({
			...
			scopes: ['okta.apps.read', 'okta.authorizationServers.read'],
			privateKey: jwks
		});

		appInfo = await get_application_data(client, origRequest.client_id)
		scopeDefinitions = await get_scope_data(client, origRequest.scope)
	}
	catch(error) {
		console.log(error)
		return {
...
async function get_application_data(client, client_id) {
	...
	var appName = oktaResponse.label
	var appIcon = null
	if (oktaResponse._links.logo) {
		appIcon = oktaResponse._links.logo[0].href
	}
	var returnValue = {
...
async function get_scope_data(client, clientRequestedScopes) {
	console.log('Retrieving Scope data from Okta.')
	var returnScopes = []

	await client.listOAuth2Scopes(process.env.AUTHZ_SERVER, {}).each(scope => {
